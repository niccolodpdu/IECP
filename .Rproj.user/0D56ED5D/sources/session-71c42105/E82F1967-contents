...
#' Dynamically extract the A matrix from the output of deconvolution function based on a user-specified address
#' @export
extract_element <- function(data, address) {
  # Construct the full expression as a string
  expression_text <- paste0("data", address)
  
  # Evaluate the expression in a safe environment
  result <- tryCatch({
    eval(parse(text = expression_text))
  }, error = function(e) {
    cat("Error in accessing the data with provided address:", address, "\n", e$message, "\n")
...
#' Helper function specifically helping deconvolution methods that require feature selection based on variance ranking
#' @export
select_most_variant <- function(input, k) {
  variance <- apply(input, 1, var, na.rm = TRUE)
  most_variant <- names(variance[order(variance, decreasing = TRUE)][1:k])
  return(most_variant)
}
